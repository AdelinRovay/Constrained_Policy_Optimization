...
from stable_baselines3 import SAC
from stable_baselines3.common.vec_env import SubprocVecEnv
from envs.custom_humanoid_env import CustomTerrainHumanoidEnv
from stable_baselines3.common.vec_env import DummyVecEnv
from tqdm import tqdm
import matplotlib.pyplot as plt
import pickle
...
log_dir = "E:/mujoco_project/logs/human_logs"
model_dir = "E:/mujoco_project/model/human_model"
...
def train(env, sb3_algo, total_timesteps=1000):
    ...
    while iters < max_iters:
        iters += 1
        with tqdm(total=TIMESTEPS, desc=f'Iteration {iters}/{max_iters}', unit='timesteps') as pbar:
            model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, callback=lambda loc, glob: custom_reward_human(loc, glob, env))
            pbar.update(TIMESTEPS)
    ...
    logs_path = 'logs_RL_human_SAC.pkl'
    with open(logs_path, 'rb') as f:
        logs = pickle.load(f)
        print(logs)
        objective_speed.extend(logs['speed'])
...
if __name__ == "__main__":
    sb3_algo = 'SAC'
    total_timesteps = 500
    env_name = 'CustomTerrainHumanoid-v0'
    train(env_name, sb3_algo, total_timesteps)
